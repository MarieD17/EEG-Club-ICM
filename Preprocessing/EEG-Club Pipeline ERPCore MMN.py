...
if 'thandrillon' in os.getcwd():
    ...
    path_data='/Users/marie.degrave/Documents/EEG-Club perso/MMN_ERP/'

if os.path.exists(path_data+"reports")==False:
    os.makedirs(path_data+"reports")
if os.path.exists(path_data+"intermediary")==False:
    os.makedirs(path_data+"intermediary")                                       # create the directory if it doesn't exist 
...
for file in files:
    ...
    # [2] MONTAGE  -  Create a montage to maps channels to a layout
    ...
    report.add_raw(raw=raw_eeg, title='Filtered Cont from"raw_eeg"', 
                   psd=False)                                                   # Add raw object to the report, omit PSD plot
    ...
    ica_fig=ica.plot_sources(
        ica_epochs_clean, show_scrollbars=True, show=False
    ...
    face_vis = mne.combine_evoked(
        [evoked_face, evoked_car], weights=[1, -1]
        )
